...
https://gitlab.gwdg.de/irp/holotomotoolbox/-/blob/master/functions/generators/simulateHologram.m?ref_type=heads

"""

import numpy as np
from SSPR.Propagation.FresnelPropagator import FresnelPropagator
...
"""
    ...
    # fresnel_number = np.array(fresnel_number).reshape(-1, 1)

    if beam is None:
        beam = np.ones_like(phase_image)
    ...
    if simulate_linear:
        hologram = 1 + 2 * np.real(fp.forward_propagate(beam * phase_abs_image))
    else:
        hologram = np.abs(fp.forward_propagate(beam * np.exp(phase_abs_image))) ** 2
