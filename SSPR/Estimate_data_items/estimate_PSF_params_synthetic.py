...
Using the "estimate_PSF_params_exp.py" code we can estimate the point spread function (PSF) by performing a
minimization procedure between an experimentally flat-field corrected (FFC) image and a synthetic (noiseless and
without blur and speckle) version. We can use this PSF value to deconvolve the experimental flat-field corrected
image and then perform phase retrieval.

We can use this same PSF to blur our simulated white fields and simulated static/dynamic void images that correspond
to the experimental images (as that is what we see experimentally). However, we cannot use this same PSF to deconvolve
the simulated static/dynamic void FFC images. We must assume that we do not know what the PSF value is -- the same way
we did not know what the PSF value is for the experimental images until after we performed the minimization procedure.
So this code does the same minimization procedure between a synthetic (noiseless and without blur and speckle) image
and a SIMULATED static FFC image (instead of an experimental static FFC image). We can use this PSF value to deconvolve
the simulated static/dynamic FFC images and then perform phase retrieval.

Before using this algorithm you must generate synthetic white fields in such a way that they match the experimental
white fields as best as possible. Then you must apply image registration the same way as with the experimental white
fields. After that, you must generate a synthetic static void hologram and apply image registration and PCA as with
the experimental void hologram. This image will be used in the minimization scheme in this script.
...
from SSPR.utilities_PyTorch import (FFT, IFFT, showImg, create_circular_mask, compare_plots_lineout, fadeoutImage,
                               GaussianSmoothing, padToSize)
...
def func(sig, gam, simulated_hologram, experimental_hologram, device='cpu'):
    ...
    # convolved_hologram, _ = fadeoutImage(img=convolved_hologram,
    #                                      fadeMethod='ellipse',
    #                                      ellipseSize=[0.55, 0.55],
    #                                      transitionLength=[20, 20],
    #                                      fadeToVal=1.0)
    convolved_hologram = convolved_hologram * mask
    experimental_hologram = experimental_hologram * mask
    # showImg(convolved_hologram.detach(), clim=(0, 2))
    # showImg(experimental_hologram.detach())
    loss = torch.sum(abs(convolved_hologram - experimental_hologram) ** 2)  # Example: MSE loss
    # loss = cc_loss(convolved_hologram, experimental_hologram)
    print(loss)
...
tiff_holo_ffc_syn = "run" + run_holo + "_sim_holos_with_speckle_FFC.tiff"

# Import experimental hologram
holo_syn = torch.tensor(imread(dir_main + dir_sim + tiff_holo_ffc_syn), dtype=torch.float32)

holo_syn, _ = fadeoutImage(img=holo_syn,
                           fadeMethod='ellipse',
                           fadeToVal=1.0,
                           transitionLength=[20, 20],
...
d_void_out = torch.tensor(40.707 * um, dtype=torch.float32)
t_shell = torch.tensor(1.449 * um, dtype=torch.float32)
SU8_thickness = torch.tensor(400 * um, dtype=torch.float32)

# Create mask
mask = create_circular_mask(size=holo_syn.shape[0], percentage=0.85, smooth_pixels=20)
...
sigma = torch.tensor(350e-9, dtype=torch.float32, requires_grad=True)  # Gaussian sigma guess
gamma = torch.tensor(0e-9, dtype=torch.float32, requires_grad=True)  # Lorentzian gamma guess
x_c = torch.tensor(-1.944e-6, dtype=torch.float32, requires_grad=True)
y_c = torch.tensor(-7.043e-6, dtype=torch.float32, requires_grad=True)
...
learning_rates = {'sigma': 50e-9,
                  'gamma': 0e-9,
                  'x_c': 0.05e-6,  # was 0.1e-6
                  'y_c': 0.05e-6,  # was 0.1e-6
                  'd_void_out': 0.05e-6,  # was 0.5e-6
                  't_shell': 0.05e-6}  # was 0.5e-6
...
params = torch.tensor([sigma, gamma, d_void_out, t_shell, x_c, y_c],
                      dtype=torch.float32,
                      device=device,
                      requires_grad=True)
optimizer = torch.optim.Adam(params=param_list)

# sigma_min, sigma_max = 50e-9, 700e-9
# gamma_min, gamma_max = 5e-9, 150e-9
...
for i in tqdm(range(iterations)):
    ...
    arg_in = (r_void_in ** 2 - (x - x_c_new) ** 2 - (y - y_c_new) ** 2)  # Argument for spherical bead (smaller than tbead)
    arg_in[arg_in < 0] = 0
    ...
    mu = k0 * (SU8 * beta_SU8 + t_void * beta_SiO2)
    ...
    loss, conv_holo = func(sig=sigma_new,
                           ...
                           experimental_hologram=holo_syn)
