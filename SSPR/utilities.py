...
#     dx : sequence or None
#         Grid spacing [dy, dx]
#     computeMeshgrid : bool
#         If True, return full 2D grids Y, X.
#         If False, return broadcastable arrays Y:(ny,1), X:(1,nx).
#
#     Returns
#     -------
#     Y, X
...
#     dy, dx_ = dx[0], dx[1]
#
#     y = (np.arange(-ny / 2 + 0.5, ny / 2, 1) * dy).astype(np.float32)
#     x = (np.arange(-nx / 2 + 0.5, nx / 2, 1) * dx_).astype(np.float32)
#
#     if computeMeshgrid:
#         Y, X = np.meshgrid(y, x, indexing='ij')   # both (ny, nx)
...
# Old one I used where y and x were flipped?
def centeredGrid(N, dx=None, computeMeshgrid=False):
    """
    centeredGrid creates a centered coordinate-grid of given size.
...
def padFadeOut(img, outputSize, transitionLength=None, padValue=None, fadeoutParallel=False, computeMeshgrid=False):
    ...
    # transitionMask = transitionMask.T
...
def make_Gaussian(size, fwhm=1200, center=None, exponent=1.0):
    ...
    exponent: float
        A factor to control the sharpness of the falloff. Higher values result in faster falloff.
    """
