...
if type == "sim":
    ...
    # ph = ph[:len_y-abs(shift_y), :len_x-abs(shift_x)]
    # ph_gt = ph_gt[:len_y-abs(shift_y), :len_x-abs(shift_x)]
    # I_sim = I_sim[:len_y-abs(shift_y), :len_x-abs(shift_x)]
    # I_sim_raw = I_sim_raw[:len_y-abs(shift_y), :len_x-abs(shift_x)]
    ph = ph[abs(shift_y):len_y, abs(shift_x):]
    ph_gt = ph_gt[abs(shift_y):len_y, abs(shift_x):]
    I_sim = I_sim[abs(shift_y):len_y, abs(shift_x):]
    I_sim_raw = I_sim_raw[abs(shift_y):len_y, abs(shift_x):]
else:
    ph = np.array(imread(dir_main + "run" + run + "_" + type + ".tiff"))
    len_x = np.shape(ph)[1]
    len_y = np.shape(ph)[0]
    ...
    # ph = ph[:len_y-abs(shift_y), :len_x-abs(shift_x)]
    # I_exp = I_exp[:len_y-abs(shift_y), :len_x-abs(shift_x)]
    # I_exp_raw = I_exp_raw[:len_y-abs(shift_y), :len_x-abs(shift_x)]
    ph = ph[abs(shift_y):len_y, abs(shift_x):]
    I_exp = I_exp[abs(shift_y):len_y, abs(shift_x):]
    I_exp_raw = I_exp_raw[abs(shift_y):len_y, abs(shift_x):]
...
if type == "sim":
    ...
    # imwrite("/Users/danielhodge/Desktop/run" + run + "_" + type + "_mu_gt_final.tiff", mu_gt)
else:
