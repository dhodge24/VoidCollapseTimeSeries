...
for run in runs:
    # ----------------
    # SIM reconstruction paths
    # ----------------
    ped_sim_dir = dir_main + dir_ped + f"run{run}_sim/"
    areal_sim_dir = dir_main + dir_areal + f"run{run}_sim/"

    # ----------------
    # RECON FILES
    # ----------------
    ped_recon_path = ped_sim_dir + f"proj_elec_density_run{run}sim.tiff"
    areal_recon_path = areal_sim_dir + f"areal_density_run{run}sim.tiff"
...
# ----------------
plt.savefig(
    "/Users/danielhodge/Desktop/5x2_PED_Areal_sim_vs_GT.pdf",
    dpi=300,
    bbox_inches="tight",
    transparent=False
)
