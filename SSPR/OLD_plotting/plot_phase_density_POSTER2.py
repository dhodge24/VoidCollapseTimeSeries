...
# ----------------
# ORGANIZE AS 4 IMAGE ROWS x 8 COLS
# ----------------
images_grid = [
    [
    ...
    [
        img1_sim_proj_elec_dens, img1_exp_proj_elec_dens,
        img2_sim_proj_elec_dens, img2_exp_proj_elec_dens,
        img3_sim_proj_elec_dens, img3_exp_proj_elec_dens,
        img4_sim_proj_elec_dens, img4_exp_proj_elec_dens
    ],
    [
...
delay_times = [6.82, 8.96, 10.52, 13.42]
...
# CREATE FIGURE
# ----------------
fig, axs = plt.subplots(5, 8, figsize=(20, 12), constrained_layout=False)
...
# ----------------
# ADD RED DASHED CENTER LINE TO 4TH ROW IMAGES
# ----------------
for c in range(8):
    ax = axs[3, c]
    img = images_grid[3][c]
...
# ----------------
# 5TH ROW: VERTICAL LINEOUTS, PLOTTED HORIZONTALLY
# x-axis = pixel
# y-axis = areal density
# flip lineouts so they are rightside up
# ----------------
recon_areal_list = [
    img1_sim_areal_density, img1_exp_areal_density,
    img2_sim_areal_density, img2_exp_areal_density,
...
for c in range(8):
    ...
    # vertical lineout from center column of image
    # flip upside-down issue by reversing the pixel order
    ...
    ax.plot(pixels_gt, gt_line, color='black', linewidth=1.4, label="Ground Truth")
    ...
    # remove y tick labels except first panel
    if c != 0:
        ax.set_yticklabels([])
    else:
        ax.set_ylabel(
    ...
    # red dashed marker in the lineout plots to match image center-line styling
    ax.axhline(
        y=0,
        alpha=0
    )
...
# ----------------
plt.savefig(
    '/Users/danielhodge/Desktop/exp_sim_5x8_plots.pdf',
    dpi=300,
    transparent=False,
    bbox_inches='tight'
)
